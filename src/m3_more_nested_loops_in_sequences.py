...
def run_test_largest_number():
    ...
    # Test 4:
    expected = -3
    answer = largest_number(([-5,-8,-3], [-100], [-10000,-500,-90]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for i in range(len(seq_seq[k])):
            if len(seq_seq[k]) == 0:
                pass
            elif seq_seq[k][i]>seq_seq[maxindk][maxindi]:
                maxindi=i
                maxindk=k
    return seq_seq[maxindk][maxindi]
...
def run_test_largest_negative_number():
    ...
    # Test 2:
    expected = -1111111111111111
    answer = largest_negative_number(([], [-1111111111111111], [-111111, 11111]))
    print('Expected and actual are:', expected, answer)

    # Test 3:
    expected = None
    answer = largest_negative_number(([], [500], []))
    print('Expected and actual are:', expected, answer)

    # Test 4:
    expected = -10000
    answer = largest_negative_number(([-5,8,-3], [], [-10000,500,-90]))
    print('Expected and actual are:', expected, answer)
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        for l in range(1,len(seq_seq)):
            for m in range(len(seq_seq[l])):
                if seq_seq[l][m] == seq_seq[0][k]:
                    return True
    return False
